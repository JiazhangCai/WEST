...
class west(object):
    ...
    def preprocess(self, 
                    ...
                    n_top_genes = 1000, # number of remained genes with highest number of counts
    ...
    def train(self, 
                combine = True, # whether combine two ensemble methods
        ...
        if combine:
            values = adata.obs["SpaGCN"].to_numpy()
            label_encoder = LabelEncoder()
            integer_encoded = label_encoder.fit_transform(values)
            onehot_encoder = OneHotEncoder(sparse=False)
            integer_encoded = integer_encoded.reshape(len(integer_encoded), 1)
            onehot_encoded = onehot_encoder.fit_transform(integer_encoded)
            clutser_adj = onehot_encoded.dot(onehot_encoded.T)
            new_adj = copy.deepcopy(np.multiply(sf.spatial_graph.toarray(), clutser_adj))
            new_adj = sp.csc_array(new_adj)
            sf.spatial_graph = new_adj
    ...
    def embedding(self, 
        ...
        distance_matrix = np.array(self.adata.obsp["WNN"])
